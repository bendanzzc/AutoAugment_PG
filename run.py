...
class Controller:
    def __init__(self):
        ...
        self.Types = tf.placeholder(tf.int32,(30))
        for i, output in enumerate(self.model.outputs):
            types = tf.slice(self.Types, [i], [1])
            loss += tf.reduce_sum(-tf.log(output) * tf.one_hot(indices=types, depth = output.shape[2]))
        loss = loss/30 * self.scale
        #self.optimizer = tf.train.GradientDescentOptimizer(0.00035).apply_gradients(self.grads)
        self.optimizer = tf.train.AdamOptimizer(0.00035).minimize(loss)
